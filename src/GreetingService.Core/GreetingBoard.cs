...
{
    /// <summary>
    /// A "Name"/"Greeting" header followed by a row per name — the name, then the greeting it
    /// produces — rendered by <see cref="TextTable"/> so every caller lays the columns out the same way.
    /// </summary>
